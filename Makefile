PY=/usr/bin/python
NOSE=/usr/bin/nosetests1.1 -s -v --with-xunit --with-coverage --cover-erase --cover-package configa
NOSE_ENV=.env/bin/nosetests -s -v --with-xunit --with-coverage --cover-erase --cover-package configa
GIT=/usr/bin/git
...
TEST=configa.tests:TestConfig \
	configa.tests:TestSetter
...
uninstall:
	$(RPM) -e python-configa

install:
	$(RPM) -ivh dist/python-configa-?.??-?.noarch.rpm

upgrade:
	$(RPM) -Uvh dist/python-configa-?.??-?.noarch.rpm
